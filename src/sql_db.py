...
class SQL:
    ...
    def write(self, process_details: ProcessDetails):
        ...
        try:
            self.sql.execute('create table activity(pid int, title varchar(256), exe_name varchar(256), process_start datetime(6), process_end datetime(6), runtime time(6))')
        except mysql.connector.errors.ProgrammingError:
            pass

        self.sql.execute(insert_cmd, vals)
        self.db_main.commit()
